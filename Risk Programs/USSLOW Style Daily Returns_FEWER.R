...
#Beta, Dividend.Yield,Earnings.Quality,Earnings.Yield,Growth,Leverage,Liquidity,
#Long.Term.Reversal,Management.Quality,Mid.Capitalization,Momentum,
...
styles_to_graph<-c("Earnings.Quality","Management.Quality","Profitability")

#Get data and set date class
rawdailyreturns<-read.csv("USSLOW_Style_Daily_Returns.csv")
rawdailyreturns$Dates<-as.Date(as.character(rawdailyreturns$Dates,"%Y/%m/%d"))
...
color_data<-USSLOW_ALL_STYLES[USSLOW_ALL_STYLES_NAMES %in% names(cumulated_returns)]
...
ggplot(melted_cuml_returns,aes(x=Dates,y=value,color=variable))+
        ...
        labs(title="USSLOW Cumulative Returns")+
